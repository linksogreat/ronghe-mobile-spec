...
window.RongHeComponents = {
        ...
        demos: [
            {
                ...
                type: 'vue',
                setupStr: "return (() => { const { ref, computed, watch } = Vue; const style = ref('basic'); const active = ref(0); const schemes = [ { key: 'basic', label: '常规', activeColor: '#1890FF', inactiveColor: '#969799', styleVars: {}, items: [{ text: '首页', icon: 'home-o' }, { text: '分类', icon: 'apps-o' }, { text: '购物车', icon: 'cart-o' }, { text: '我的', icon: 'user-o' }] }, { key: 'badge', label: '徽标', activeColor: '#1890FF', inactiveColor: '#969799', styleVars: {}, items: [{ text: '首页', icon: 'home-o' }, { text: '消息', icon: 'chat-o', badge: 9 }, { text: '待办', icon: 'todo-list-o', dot: true }, { text: '设置', icon: 'setting-o' }] }, { key: 'text', label: '纯文字', activeColor: '#1890FF', inactiveColor: '#969799', styleVars: {}, items: [{ text: '首页' }, { text: '发现' }, { text: '消息', badge: 5 }, { text: '我的' }] }, { key: 'center', label: '中间按钮', activeColor: '#1890FF', inactiveColor: '#969799', styleVars: {}, items: [{ text: '首页', icon: 'home-o' }, { text: '订单', icon: 'notes-o' }, { text: '发布', type: 'center' }, { text: '消息', icon: 'chat-o', dot: true }, { text: '我的', icon: 'user-o' }] }, { key: 'dark', label: '深色', activeColor: '#22c55e', inactiveColor: '#94a3b8', styleVars: { '--van-tabbar-background': '#0b1220' }, items: [{ text: '首页', icon: 'home-o' }, { text: '行情', icon: 'points' }, { text: '消息', icon: 'chat-o', badge: 2 }, { text: '我的', icon: 'user-o' }] } ]; const currentScheme = computed(() => schemes.find(s => s.key === style.value) || schemes[0]); const currentText = computed(() => currentScheme.value.items[active.value]?.text || ''); const onCenterClick = () => vant.showToast('发布'); watch(style, () => { active.value = 0; }); return { style, active, schemes, currentScheme, currentText, onCenterClick }; })();",
                template: `<div class="h-full min-h-0 bg-[#f7f8fa] flex flex-col overflow-hidden">
                    <div class="bg-white">
                        <van-tabs v-model:active="style" color="#1890FF" swipeable>
                            <van-tab v-for="s in schemes" :key="s.key" :name="s.key" :title="s.label"></van-tab>
                        </van-tabs>
                    </div>

                    <div class="flex-1 min-h-0 flex items-center justify-center">
                        <div class="text-center px-6">
                            <div class="text-[26px] font-semibold text-[#323233] leading-8">{{ currentText }}</div>
                            <div class="text-xs text-[#969799] mt-2">点击底部标签切换（无中间列表内容）</div>
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        <van-tabbar
                            ...
                            :active-color="currentScheme.activeColor"
                            :inactive-color="currentScheme.inactiveColor"
                            :style="currentScheme.styleVars"
                        >
                            <template v-for="(t, i) in currentScheme.items" :key="t.text + i">
                                <van-tabbar-item
                                    v-if="t.type !== 'center'"
                                    :icon="t.icon"
                                    :badge="t.badge"
                                ...
                                <van-tabbar-item v-else @click="onCenterClick">
                                    <template #icon>
                                        <div class="w-12 h-12 rounded-full bg-[#1890FF] flex items-center justify-center -mt-6 shadow-[0_6px_18px_rgba(0,0,0,0.12)]">
                                            <van-icon name="plus" color="white" size="22"></van-icon>
                                        </div>
                                    </template>
                                    <span class="opacity-0">发布</span>
                                </van-tabbar-item>
                            </template>
                        </van-tabbar>
                    </div>
                </div>`,
                code: `<van-tabbar v-model="active" active-color="#1890FF" inactive-color="#969799">\n  <van-tabbar-item icon="home-o">首页</van-tabbar-item>\n  <van-tabbar-item icon="chat-o" badge="9">消息</van-tabbar-item>\n  <van-tabbar-item icon="setting-o" dot>设置</van-tabbar-item>\n</van-tabbar>`
            },
            {
                title: '常规（图标+文字）',
                type: 'vue',
                setupStr: "return { active: Vue.ref(0) };",
                template: `<div class="bg-white"><van-tabbar v-model="active" active-color="#1890FF" inactive-color="#969799"><van-tabbar-item icon="home-o">首页</van-tabbar-item><van-tabbar-item icon="apps-o">分类</van-tabbar-item><van-tabbar-item icon="cart-o">购物车</van-tabbar-item><van-tabbar-item icon="user-o">我的</van-tabbar-item></van-tabbar></div>`,
                code: `<van-tabbar v-model="active">\n  <van-tabbar-item icon="home-o">首页</van-tabbar-item>\n</van-tabbar>`
            },
            {
                title: '徽标（badge/dot）',
                type: 'vue',
                setupStr: "return { active: Vue.ref(1) };",
                template: `<div class="bg-white"><van-tabbar v-model="active" active-color="#1890FF" inactive-color="#969799"><van-tabbar-item icon="home-o">首页</van-tabbar-item><van-tabbar-item icon="chat-o" badge="9">消息</van-tabbar-item><van-tabbar-item icon="todo-list-o" dot>待办</van-tabbar-item><van-tabbar-item icon="setting-o">设置</van-tabbar-item></van-tabbar></div>`,
                code: `<van-tabbar-item icon="chat-o" badge="9">消息</van-tabbar-item>\n<van-tabbar-item icon="todo-list-o" dot>待办</van-tabbar-item>`
            },
            {
                title: '纯文字（无 icon）',
                type: 'vue',
                setupStr: "return { active: Vue.ref(0) };",
                template: `<div class="bg-white"><van-tabbar v-model="active" active-color="#1890FF" inactive-color="#969799"><van-tabbar-item>首页</van-tabbar-item><van-tabbar-item>发现</van-tabbar-item><van-tabbar-item badge="5">消息</van-tabbar-item><van-tabbar-item>我的</van-tabbar-item></van-tabbar></div>`,
            ...
            },
            {
                ...
                type: 'vue',
                setupStr: "return { active: Vue.ref(0) };",
                template: `<div class="bg-white"><van-tabbar v-model="active" :style=\"{ '--van-tabbar-background': '#0b1220' }\" active-color="#22c55e" inactive-color="#94a3b8"><van-tabbar-item icon="home-o">首页</van-tabbar-item><van-tabbar-item icon="points">行情</van-tabbar-item><van-tabbar-item icon="chat-o" badge="2">消息</van-tabbar-item><van-tabbar-item icon="user-o">我的</van-tabbar-item></van-tabbar></div>`,
                code: `<van-tabbar :style="{ '--van-tabbar-background': '#0b1220' }" active-color="#22c55e" inactive-color="#94a3b8">...</van-tabbar>`
            }
        ],
        rules: [
            '<strong>基础结构</strong>：固定在页面底部，建议配合 placeholder + safe-area-inset-bottom；点击切换对应页面或内容区。',
            '<strong>类型</strong>：常规（图标+文字）、徽标（badge/dot）、纯文字、带中间操作按钮、深色主题等；同一产品内建议统一 Tab 数量与图标风格。',
            '<strong>颜色</strong>：active-color 用品牌主色；inactive-color 用中性灰；主题化时同时控制背景色（--van-tabbar-background）。',
            '<strong>徽标</strong>：badge 用于数字提醒，dot 用于轻提示；避免在所有 Tab 同时展示徽标造成噪音。'
        ]
    },
    'grid-nav': {
        name: '4.7 宫格',
        ...
        demos: [
            {
                ...
                type: 'vue',
                setupStr:
                    "return (() => { const showTop = Vue.ref(false); const showCenter = Vue.ref(false); const showBottom = Vue.ref(false); const showSide = Vue.ref(false); const open = (pos) => { if (pos === 'top') showTop.value = true; if (pos === 'center') showCenter.value = true; if (pos === 'bottom') showBottom.value = true; if (pos === 'side') showSide.value = true; }; return { showTop, showCenter, showBottom, showSide, open }; })();",
                template: `
                    <div class="h-full min-h-0 bg-[#f7f8fa] flex flex-col overflow-hidden">
                        <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar py-3">
                            <div class="px-4 text-xs text-[#969799] mb-2">触发入口（页面级示例）</div>
                            <van-cell-group inset :border="false">
                                <van-cell title="顶部弹出" value="position=top" is-link clickable @click="open('top')" />
                                <van-cell title="居中弹出" value="默认居中" is-link clickable @click="open('center')" />
                                <van-cell title="底部弹出" value="position=bottom" is-link clickable @click="open('bottom')" />
                                <van-cell title="侧边弹出" value="position=right" is-link clickable @click="open('side')" />
                            </van-cell-group>
                            ...
                            <div class="px-4 text-xs text-[#969799] mb-2">说明</div>
                            <div class="px-4 text-sm text-[#323233] leading-6">
                                弹窗层用于承载临时浮层内容，支持不同方向出现；本示例仅展示“内容区域容器”，不包含具体业务内容。点击遮罩关闭。
                            </div>
                        </div>

                        <van-popup v-model:show="showTop" position="top">
                            <div class="bg-white" style="height: 180px;"></div>
                        </van-popup>

                        <van-popup v-model:show="showCenter" round>
                            <div class="bg-white rounded-xl" style="width: 280px; height: 180px;"></div>
                        </van-popup>

                        <van-popup v-model:show="showBottom" position="bottom" round>
                            <div class="bg-white" style="height: 260px; padding-bottom: calc(env(safe-area-inset-bottom));"></div>
                        </van-popup>

                        <van-popup v-model:show="showSide" position="right" :style="{ width: '72vw', maxWidth: '320px', height: '100%' }">
                            <div class="h-full bg-white"></div>
                        </van-popup>
                    </div>
                `,
                code: `<!-- 顶部/底部/侧边 -->\n<van-popup v-model:show=\"show\" position=\"top|bottom|left|right\">\n  <div style=\"height: 200px;\"></div>\n</van-popup>\n\n<!-- 居中（默认） -->\n<van-popup v-model:show=\"show\" round>\n  <div style=\"width: 280px; height: 180px;\"></div>\n</van-popup>`
            }
        ],
        rules: [
            '<strong>定位</strong>：顶部/底部弹窗适合临时提示、表单补充；侧边弹窗适合筛选/详情；居中弹窗适合轻量提示或短流程确认。',
            '<strong>内容区</strong>：弹窗层应只承载“容器”，内容可滚动但结构需稳定；长内容建议内部滚动，避免挤压页面。',
            '<strong>关闭</strong>：默认支持点击遮罩关闭；关键流程可增加显式关闭入口（关闭按钮/返回手势），避免用户迷失。',
            '<strong>尺寸建议</strong>：居中弹窗建议宽 260~320px；侧边弹窗建议 72%~86% 视口宽度并设置 maxWidth；底部弹窗建议设置最大高度并考虑 safe-area-inset-bottom。',
            '<strong>层级与遮罩</strong>：遮罩用于聚焦当前任务，避免叠层过多；同一时刻建议只出现一个弹窗层。'
        ]
    },
    'dialog-modal': {
        name: '6.1 对话弹窗',
        ...
        demos: [
            {
                title: '页面级示例',
                type: 'vue',
                setupStr: "return (() => { const { ref, reactive, computed, nextTick, onMounted, onBeforeUnmount } = Vue; const rootEl = ref(null); const scrollEl = ref(null); const stepIndex = ref(0); const padding = 8; const highlight = reactive({ top: 0, left: 0, width: 0, height: 0 }); const steps = [ { key: 'create', selector: '[data-guide=\"create\"]', title: '第一步：创建', desc: '点击高亮的“新建”按钮，开始创建一条数据。' }, { key: 'filter', selector: '[data-guide=\"filter\"]', title: '第二步：筛选', desc: '点击右上角“筛选”，快速定位你关注的数据。' }, { key: 'submit', selector: '[data-guide=\"submit\"]', title: '第三步：提交', desc: '点击底部“保存”，提交并完成本次操作。' } ]; const isGuiding = computed(() => stepIndex.value > 0); const currentStep = computed(() => steps[stepIndex.value - 1] || null); const clamp = (n, min, max) => Math.max(min, Math.min(max, n)); const updateHighlight = () => { if (!isGuiding.value) return; const root = rootEl.value; const st = currentStep.value; if (!root || !st) return; const target = root.querySelector(st.selector); if (!target) return; const rootRect = root.getBoundingClientRect(); const r = target.getBoundingClientRect(); const top = r.top - rootRect.top - padding; const left = r.left - rootRect.left - padding; const width = r.width + padding * 2; const height = r.height + padding * 2; highlight.top = clamp(top, 0, rootRect.height); highlight.left = clamp(left, 0, rootRect.width); highlight.width = clamp(width, 0, rootRect.width); highlight.height = clamp(height, 0, rootRect.height); }; const startGuide = async () => { stepIndex.value = 1; await nextTick(); updateHighlight(); }; const endGuide = () => { stepIndex.value = 0; }; const nextStep = async () => { if (stepIndex.value >= steps.length) { endGuide(); return; } stepIndex.value += 1; await nextTick(); updateHighlight(); }; const prevStep = async () => { if (stepIndex.value <= 1) return; stepIndex.value -= 1; await nextTick(); updateHighlight(); }; const handleTargetClick = async (key) => { if (!isGuiding.value) { vant.showToast('已点击：' + key); return; } if (currentStep.value && currentStep.value.key === key) { await nextStep(); return; } }; const maskRects = computed(() => { const root = rootEl.value; if (!root || !isGuiding.value) return null; const rootRect = root.getBoundingClientRect(); const topH = highlight.top; const bottomTop = highlight.top + highlight.height; const leftW = highlight.left; const rightLeft = highlight.left + highlight.width; return { containerW: rootRect.width, containerH: rootRect.height, top: { top: 0, left: 0, width: rootRect.width, height: topH }, bottom: { top: bottomTop, left: 0, width: rootRect.width, height: Math.max(0, rootRect.height - bottomTop) }, left: { top: highlight.top, left: 0, width: leftW, height: highlight.height }, right: { top: highlight.top, left: rightLeft, width: Math.max(0, rootRect.width - rightLeft), height: highlight.height } }; }); const tooltip = computed(() => { const root = rootEl.value; if (!root || !isGuiding.value) return null; const rootRect = root.getBoundingClientRect(); const w = 260; const margin = 12; const preferTop = highlight.top > rootRect.height * 0.6; const top = preferTop ? highlight.top - margin - 120 : highlight.top + highlight.height + margin; const left = clamp(highlight.left, 12, rootRect.width - w - 12); return { top: clamp(top, 12, rootRect.height - 12 - 120), left, width: w, preferTop }; }); const arrow = computed(() => { if (!tooltip.value) return null; const size = 10; const x = clamp(highlight.left + 16, tooltip.value.left + 16, tooltip.value.left + tooltip.value.width - 16); const y = tooltip.value.preferTop ? tooltip.value.top + 110 : tooltip.value.top - size / 2; return { left: x, top: y, size }; }); const onResize = () => updateHighlight(); onMounted(() => { window.addEventListener('resize', onResize); }); onBeforeUnmount(() => { window.removeEventListener('resize', onResize); }); return { rootEl, scrollEl, stepIndex, steps, isGuiding, currentStep, highlight, maskRects, tooltip, arrow, startGuide, endGuide, nextStep, prevStep, handleTargetClick, updateHighlight }; })();",
                template: `
                    <div ref="rootEl" class="h-full min-h-0 bg-[#f7f8fa] flex flex-col overflow-hidden relative">
                        <div ref="scrollEl" class="flex-1 min-h-0 overflow-y-auto custom-scrollbar py-3" @scroll.passive="updateHighlight">
                            <div class="px-4">
                                <div class="bg-white rounded-xl border border-[#ebedf0] p-4">
                                    <div class="text-base font-medium text-[#323233]">蒙层引导页（新功能教学）</div>
                                    <div class="text-sm text-[#646566] mt-1 leading-6">
                                        点击“开始引导”后，会出现黑色遮罩层，并高亮当前需要点击的操作入口，按步骤逐步引导完成操作。
                                    </div>
                                    <div class="mt-3">
                                        <van-button type="primary" color="#1890FF" block @click="startGuide">开始引导</van-button>
                                    </div>
                                </div>
                            </div>

                            <div class="px-4 mt-4">
                                <div class="bg-white rounded-xl border border-[#ebedf0] overflow-hidden">
                                    <div class="flex items-center justify-between px-4 h-12 border-b border-[#f7f7f7]">
                                        <div class="text-sm font-medium text-[#323233]">数据列表</div>
                                        <div class="flex items-center gap-2">
                                            <van-button size="small" plain color="#1890FF" class="!h-8" data-guide="filter" @click="handleTargetClick('filter')">筛选</van-button>
                                            <van-button size="small" type="primary" color="#1890FF" class="!h-8" data-guide="create" @click="handleTargetClick('create')">新建</van-button>
                                        </div>
                                    </div>
                                    <div class="divide-y divide-[#f7f7f7]">
                                        <div class="px-4 py-3 flex items-center justify-between">
                                            <div class="flex flex-col">
                                                <div class="text-[14px] text-[#323233]">示例数据 A</div>
                                                <div class="text-[12px] text-[#969799] mt-1">更新时间：今天 10:30</div>
                                            </div>
                                            <van-tag type="primary">进行中</van-tag>
                                        </div>
                                        <div class="px-4 py-3 flex items-center justify-between">
                                            <div class="flex flex-col">
                                                <div class="text-[14px] text-[#323233]">示例数据 B</div>
                                                <div class="text-[12px] text-[#969799] mt-1">更新时间：昨天 18:20</div>
                                            </div>
                                            <van-tag type="success">已完成</van-tag>
                                        </div>
                                        <div class="px-4 py-3 flex items-center justify-between">
                                            <div class="flex flex-col">
                                                <div class="text-[14px] text-[#323233]">示例数据 C</div>
                                                <div class="text-[12px] text-[#969799] mt-1">更新时间：2026-05-01</div>
                                            </div>
                        ...
                        <div class="p-3 bg-white border-t border-[#ebedf0]">
                            <van-button type="primary" color="#1890FF" block data-guide="submit" @click="handleTargetClick('submit')">保存</van-button>
                        </div>

                        <div v-if="isGuiding && maskRects" class="absolute inset-0 z-[60]">
                            <div class="absolute bg-black/70" :style="{ top: maskRects.top.top + 'px', left: maskRects.top.left + 'px', width: maskRects.top.width + 'px', height: maskRects.top.height + 'px' }"></div>
                            <div class="absolute bg-black/70" :style="{ top: maskRects.bottom.top + 'px', left: maskRects.bottom.left + 'px', width: maskRects.bottom.width + 'px', height: maskRects.bottom.height + 'px' }"></div>
                            <div class="absolute bg-black/70" :style="{ top: maskRects.left.top + 'px', left: maskRects.left.left + 'px', width: maskRects.left.width + 'px', height: maskRects.left.height + 'px' }"></div>
                            <div class="absolute bg-black/70" :style="{ top: maskRects.right.top + 'px', left: maskRects.right.left + 'px', width: maskRects.right.width + 'px', height: maskRects.right.height + 'px' }"></div>

                            <div class="absolute pointer-events-none rounded-xl border-2 border-white shadow-[0_0_0_2px_rgba(24,144,255,0.25)]" :style="{ top: highlight.top + 'px', left: highlight.left + 'px', width: highlight.width + 'px', height: highlight.height + 'px' }"></div>

                            <button class="absolute top-3 right-3 text-white text-[13px] px-3 h-8 rounded-full bg-white/10 active:bg-white/20" @click.stop="endGuide">跳过</button>

                            <div v-if="tooltip && currentStep" class="absolute" :style="{ top: tooltip.top + 'px', left: tooltip.left + 'px', width: tooltip.width + 'px' }">
                                <div class="bg-white rounded-xl px-4 py-3 shadow-lg">
                                    <div class="text-[14px] font-medium text-[#323233]">{{ currentStep.title }}</div>
                                    <div class="text-[12px] text-[#646566] leading-5 mt-1">{{ currentStep.desc }}</div>
                                    <div class="flex items-center justify-between mt-3">
                                        <div class="text-[12px] text-[#969799]">{{ stepIndex }}/{{ steps.length }}</div>
                                        <div class="flex items-center gap-3">
                                            <button v-if="stepIndex > 1" class="text-[12px] text-[#646566]" @click.stop="prevStep">上一步</button>
                                            <button class="text-[12px] text-[#1890FF]" @click.stop="nextStep">{{ stepIndex === steps.length ? '完成' : '下一步' }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div v-if="arrow" class="absolute w-3 h-3 bg-white rotate-45 shadow-[0_2px_10px_rgba(0,0,0,0.12)]" :style="{ top: arrow.top + 'px', left: arrow.left + 'px' }"></div>
                        </div>
                    </div>
                `,
                code: `<div class="relative">\n  <!-- 页面内容 -->\n  <div v-if="step > 0" class="absolute inset-0">\n    <!-- 四块遮罩(top/bottom/left/right) 留出高亮镂空区域 -->\n  </div>\n</div>`
            }
        ],
        rules: [
            '<strong>使用场景</strong>：新功能上线教学、关键路径引导、降低首次使用门槛；适用于 2~5 步的短流程。',
        ...
        demos: [
            {
                ...
                type: 'vue',
                setupStr:
                    "return { thumb: 'data:image/svg+xml;charset=UTF-8,%3Csvg%20width%3D%22100%22%20height%3D%22100%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Crect%20width%3D%22100%25%22%20height%3D%22100%25%22%20fill%3D%22%23eeeeee%22%2F%3E%3Ctext%20x%3D%2250%25%22%20y%3D%2250%25%22%20font-family%3D%22sans-serif%22%20font-size%3D%2214%22%20fill%3D%22%23999%22%20text-anchor%3D%22middle%22%20dy%3D%22.3em%22%3EImage%3C%2Ftext%3E%3C%2Fsvg%3E' };",
                template: `<div class="h-full min-h-0 bg-[#f7f8fa] p-4">
  <van-card num="2" price="2.00" desc="描述信息" title="商品标题" :thumb="thumb" />
</div>`,
                code: '<van-card num="2" price="2.00" desc="描述信息" title="商品标题" thumb="..." />'
            },
            {
                title: '标签 + 操作区',
                type: 'vue',
                setupStr:
                    "return { thumb: 'data:image/svg+xml;charset=UTF-8,%3Csvg%20width%3D%22100%22%20height%3D%22100%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Crect%20width%3D%22100%25%22%20height%3D%22100%25%22%20fill%3D%22%23eeeeee%22%2F%3E%3Ctext%20x%3D%2250%25%22%20y%3D%2250%25%22%20font-family%3D%22sans-serif%22%20font-size%3D%2214%22%20fill%3D%22%23999%22%20text-anchor%3D%22middle%22%20dy%3D%22.3em%22%3EImage%3C%2Ftext%3E%3C%2Fsvg%3E', onAdd: () => vant.showToast('加入购物车'), onBuy: () => vant.showToast('立即购买') };",
                template: `<div class="h-full min-h-0 bg-[#f7f8fa] p-4">
  <van-card
    num="1"
    price="128.00"
  ...
  >
    <template #tags>
      <van-tag plain type="danger" class="mr-1">限时</van-tag>
      <van-tag plain type="primary">包邮</van-tag>
    </template>
    <template #footer>
      <van-button size="mini" plain type="primary" color="#1890FF" @click="onAdd">加入</van-button>
      <van-button size="mini" type="primary" color="#1890FF" class="ml-2" @click="onBuy">购买</van-button>
    </template>
  </van-card>
</div>`,
                code: '<van-card title="运动短袖" desc="轻薄透气，适合夏季" price="128.00" origin-price="199.00" tag="热销" thumb="...">\n  <template #tags>\n    <van-tag plain type="danger">限时</van-tag>\n    <van-tag plain type="primary">包邮</van-tag>\n  </template>\n  <template #footer>\n    <van-button size="mini" plain type="primary" color="#1890FF">加入</van-button>\n    <van-button size="mini" type="primary" color="#1890FF">购买</van-button>\n  </template>\n</van-card>'
            },
            {
                title: '列表选择态',
                type: 'vue',
                setupStr:
                    "return (() => { const active = Vue.ref('a'); const list = [ { id: 'a', title: '基础套餐', desc: '含基础服务与保障', price: '99.00', num: '1' }, { id: 'b', title: '升级套餐', desc: '含更多权益', price: '159.00', num: '1' } ]; const thumb = 'data:image/svg+xml;charset=UTF-8,%3Csvg%20width%3D%22100%22%20height%3D%22100%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Crect%20width%3D%22100%25%22%20height%3D%22100%25%22%20fill%3D%22%23eeeeee%22%2F%3E%3Ctext%20x%3D%2250%25%22%20y%3D%2250%25%22%20font-family%3D%22sans-serif%22%20font-size%3D%2214%22%20fill%3D%22%23999%22%20text-anchor%3D%22middle%22%20dy%3D%22.3em%22%3EImage%3C%2Ftext%3E%3C%2Fsvg%3E'; const onSelect = (id) => { active.value = id; }; return { active, list, thumb, onSelect }; })();",
                template: `<div class="h-full min-h-0 bg-[#f7f8fa] p-4">
  <div class="space-y-3">
    <div
      v-for="item in list"
      ...
      :class="active === item.id ? 'ring-2 ring-[#1890FF]' : 'ring-1 ring-[#ebedf0]'"
      @click="onSelect(item.id)"
    >
      <div class="bg-white relative">
        <div class="absolute top-3 right-3 z-10">
          <van-checkbox :model-value="active === item.id" checked-color="#1890FF" />
        </div>
        <van-card :title="item.title" :desc="item.desc" :price="item.price" :num="item.num" :thumb="thumb" />
      </div>
    </div>
  </div>
</div>`,
                code: '<div class="rounded-xl overflow-hidden" :class="active ? \'ring-2 ring-[#1890FF]\' : \'ring-1 ring-[#ebedf0]\'">\n  <van-card title="基础套餐" desc="含基础服务与保障" price="99.00" num="1" thumb="..." />\n</div>'
            },
            {
                title: '加载态（骨架切换）',
                type: 'vue',
                setupStr:
                    "return (() => { const loading = Vue.ref(true); const thumb = 'data:image/svg+xml;charset=UTF-8,%3Csvg%20width%3D%22100%22%20height%3D%22100%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Crect%20width%3D%22100%25%22%20height%3D%22100%25%22%20fill%3D%22%23eeeeee%22%2F%3E%3Ctext%20x%3D%2250%25%22%20y%3D%2250%25%22%20font-family%3D%22sans-serif%22%20font-size%3D%2214%22%20fill%3D%22%23999%22%20text-anchor%3D%22middle%22%20dy%3D%22.3em%22%3EImage%3C%2Ftext%3E%3C%2Fsvg%3E'; return { loading, thumb }; })();",
                template: `<div class="h-full min-h-0 bg-[#f7f8fa] p-4">
  <div class="bg-white rounded-xl p-3">
    <div class="flex items-center justify-between mb-3">
      <div class="text-sm font-medium text-[#333]">加载态</div>
      <van-switch v-model="loading" size="20px" active-color="#1890FF" />
    </div>
    <div v-if="loading">
    ...
    <div v-else>
      <van-card num="2" price="2.00" desc="描述信息" title="商品标题" :thumb="thumb" />
                ...
                code: '<van-switch v-model="loading" active-color="#1890FF" />\n<van-skeleton v-if="loading" title :row="2" />\n<van-card v-else title="商品标题" desc="描述信息" price="2.00" num="2" thumb="..." />'
            }
        ],
        rules: [
            '<strong>信息结构</strong>：优先保证主信息可读（标题、价格），描述信息为辅；标题建议 1-2 行，描述建议 1 行。',
            '<strong>点击反馈</strong>：当卡片可点击/可跳转时，整卡作为点击热区；避免在同一卡片内堆叠过多同级按钮。',
            '<strong>标签策略</strong>：角标/Tag 用于表达状态（热销/限时/包邮等），数量保持克制（建议 1-2 个）。',
            '<strong>行动区</strong>：操作按钮放在 footer 区，主按钮使用主题色 #1890FF；次要操作使用描边/弱化样式。',
            '<strong>列表间距</strong>：卡片列表建议外边距 16px，卡片间距 12px，圆角 12px；浅色背景下可用细描边 #ebedf0 或轻投影区分层级。',
            '<strong>状态表达</strong>：支持加载态（骨架占位）与选择态（主题色描边/高亮），状态变化应保持布局稳定。'
        ]
    },
